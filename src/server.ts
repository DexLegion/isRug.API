...
import { ASTNode, BaseASTNode, ContractDefinition, FunctionDefinition, ModifierDefinition } from "@solidity-parser/parser/dist/src/ast-types";
...
app.get("/tokens/liq", async (req, res) => {
    ...
    if (address && chain) {
        ...
        if (pairs.length > 0) {
            ...
            const locks = await findLockedLp(firstPair.base.address, firstPair.base.decimal, firstPair.pair, firstPair.totalLp, chain, net);
            liqs.push({
            ...
            if (sourceCode.SourceCode && sourceCode.ABI) {
                ...
                if (sourceCode.multiSource) {
                    ...
                    if (sourceCode.SourceCode.includes(`"sources":`)) {
                        const sources = Object.values(JSON.parse(sourceCode.SourceCode.substring(1, sourceCode.SourceCode.length - 1)).sources);
                ...
                const functionData = filteredContracts.flatMap((contract) =>
                    contract.subNodes.filter((node: FunctionDefinition): node is FunctionDefinition => node.type === "FunctionDefinition" && !!node.name)
                );
                ...
                [maxes, ownership] = await Promise.all([getMaxes(viewFuncs, intr, address, token.decimal, net), getOwner(address, net, viewFuncs, intr, true)]);
...
async function extractAddress(text: string) {
    ...
    let match: RegExpExecArray | null;
    while ((match = regex.exec(text))) {
        if (!config.burnWallets.find((x) => x == match[0]) && !config.wlContracts.find((x) => x == match[0]) && !matches.find((x) => x == match[0]))
            matches.push(match[0]);
    }
    return matches;
}
